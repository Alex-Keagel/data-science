...
def string_to_timestamp(string_time_stamp):
    return date_parser.parse(string_time_stamp)


def to_timestamp(time):
    try:
        int_time =  1000*int(string_to_timestamp(time).timestamp())
    except:
        int_time = None
    finally:
        return int_time
...
def timestamp_str_to_date_str(string_time_stamp):
    if string_time_stamp == '':
        return ''
    else:
        return date_parser.parse(string_time_stamp).date().strftime('%Y-%m-%d')
